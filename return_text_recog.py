...
avg=text_recog.textrecog()
sql="select id,name,bookname,issuedate,returndate from e where bookid='%s' and status='1';" %avg
cur.execute(sql)
result=cur.fetchall()
if result==():
    print "None"
	...
	k=[]
        ...
        if g<0:
            g=0
        print "Fine=Rs.%s"%g
        sql="update e set status='0' where bookid='%s';"%avg
        try:
             cur.execute(sql)
             db.commit()
        except:
